export class ServerChars {
    ...
    root;
    ram;
    constructor(name, skill, currentMoney, maxMoney, security, minSecurity, root, ram) {
        this.name = name;
        this.skill = skill;
    ...
    format(del = "\t") {
        return `${this.name}${del}${this.skill}` + del + this.currentMoney.toFixed() + del + this.maxMoney.toFixed() + del + (this.currentMoney * 100 / this.maxMoney).toFixed(1) + " %" + del + this.security.toFixed(2) + del + this.minSecurity.toFixed() + del + (this.security / this.minSecurity).toFixed(2) + del + this.root + del + this.ram;
    }
    get name() {
        return this.name;
    }
...
export async function main(ns) {
    ...
    list.sort((a, b) => {
        if (a.maxMoney === 0 && b.maxMoney === 0) return a.skill - b.ski;
 ...
 */
function analyzeNetwork(ns, root = 'home', list = []) {
    const server = ns.getServer(root);
    ns.getServerSecurityLevel(root);
    list[list.length] = new ServerChars(root, server.requiredHackingSkill, server.moneyAvailable, server.moneyMax,
